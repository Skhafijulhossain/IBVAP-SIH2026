  ...
  return (
    ...
    >
      ...
      <div className={`relative aspect-video w-full overflow-hidden select-none ${getSceneBackground()}`}>
        ...
        {/* Scene Simulated Graphical Graphics */}
        {camera.status === 'offline' ? (
          ...
          /* Live Animated Scene Rendering */
          <div className="absolute inset-0 pointer-events-none">
            {/* Mountain / Fence / Terrain Silhouettes */}
        ...
        {/* Top HUD Overlay: Camera ID, REC indicator, Time, FPS */}
        <div className="absolute top-2 left-2 right-2 flex items-center justify-between text-[11px] font-mono z-10 pointer-events-none">
          <div className="flex items-center gap-2">
            {/* LIVE / REC Badge */}
            <div className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-black/75 backdrop-blur-sm border border-slate-700">
              <span className={`w-2 h-2 rounded-full ${camera.status === 'online' ? 'bg-red-500 animate-ping' : 'bg-slate-500'}`} />
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="px-1.5 py-0.5 rounded bg-black/75 backdrop-blur-sm border border-slate-700 text-[10px] text-slate-300">
              <span className="text-emerald-400 font-bold">{simulatedFps}</span> FPS • {camera.resolution.split(' ')[0]}
            </div>
      ...
      <div className="px-3 py-2 bg-slate-900/90 border-t border-slate-800/80 flex items-center justify-between text-xs">
        ...
        <div className="flex items-center gap-2 text-[11px] font-mono text-slate-400">
          <span>Bitrate: <strong className="text-slate-300">{camera.bitrate}</strong></span>
          <span className="text-slate-600">|</span>
          <span className="text-slate-400">HB: {camera.lastHeartbeat}</span>
        </div>
      </div>
    </div>
  );
