        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 pl-2 pr-2.5 py-1 rounded-xl bg-[var(--surface-raised)] border border-[var(--border)] hover:border-sky-500/40 transition-all"
            >
              ...
              <div className="text-left hidden lg:block">
                ...
                <div className="text-[10px] text-sky-600 dark:text-sky-400 font-mono leading-tight">
                  {currentUser?.badgeId}
                </div>
              </div>
            ...
            {showUserDropdown && (
              <div 
                className="absolute right-0 mt-2 w-64 bg-[var(--card)] border border-[var(--border)] rounded-xl shadow-xl p-2.5 z-50 text-xs text-[var(--text)]"
                onClick={() => setShowUserDropdown(false)}
              >
                <div className="p-2 border-b border-[var(--border)] mb-2">
                  <div className="font-bold text-[var(--text)]">{currentUser?.name}</div>
                  <div className="text-[11px] text-[var(--muted)]">{currentUser?.role}</div>
                  <div className="mt-1 text-[10px] font-mono px-2 py-0.5 rounded bg-sky-500/15 text-sky-600 dark:text-sky-300 inline-block border border-sky-500/30">
                ...
                <div className="space-y-1 my-1">
                  {availableDemoUsers.map((u, i) => (
                    ...
                    >
                      ...
                      {currentUser?.id === u.id && <CheckCircle2 className="w-3.5 h-3.5 text-sky-500 shrink-0" />}
                    </button>
                  ))}
                </div>
                ...
                <div className="border-t border-[var(--border)] pt-2 mt-2">
                  <button
                    onClick={logout}
                    className="w-full text-left px-2 py-1.5 rounded-lg text-red-500 hover:bg-red-500/10 flex items-center gap-2"
                  >
